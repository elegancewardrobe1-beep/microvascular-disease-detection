...
MODEL_URL = "https://drive.google.com/uc?id=14fP-Vmc5gmX4cOvRX8yEdusw45Ai_kTo"
...
def load_model():
    if not os.path.exists(MODEL_PATH):
        with st.spinner("Downloading AI model (~50MB). Please wait..."):
            gdown.download(
                url=MODEL_URL,
                output=MODEL_PATH,
                quiet=False,
...
st.markdown(
    "<p>AI-assisted severity assessment with vascular biomarkers and explainable visual evidence.</p>",
    unsafe_allow_html=True
)
...
if uploaded:
    ...
    with col_right:
        ...
        if run_pred:
            ...
            st.markdown(
                f"<div class='label'>Confidence</div><div class='metric'>{probs[idx]*100:.2f}%</div>",
                unsafe_allow_html=True
            )
        ...
        if "cam" in st.session_state:
            ...
            overlay = overlay_cam(np.array(image), st.session_state.cam, alpha)
